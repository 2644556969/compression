...
def cryptonets_model(input):
    ...
    y = Activation(square_activation)(y)
    #adding an extra dense layer for fun 
    y = Dense(100, use_bias=True, name="fc_1half")(y) 
    y = Activation(square_activation)(y) 
    #end add 
...
def cryptonets_model_squashed(input, conv1_weights, squashed_weights, fc_1half_weights,
                              fc2_weights):
    ...
    y = Dense(
        100,
        use_bias=True,
        name="squash_fc_1half",
        trainable=False,
        kernel_initializer=tf.compat.v1.constant_initializer(
            fc_1half_weights[0]),
        bias_initializer=tf.compat.v1.constant_initializer(fc_1half_weights[1]),
    )(y)
    y = Activation(square_activation)(y)
